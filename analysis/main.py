...
print("Creating '../bok_courses.csv'...")
new_schools.to_csv('../bok_courses.csv',encoding="utf-8-sig")
  ...
  for e in ellie_cats:
    ...
    for i in range(len(ellie_cats[e])):
      ...
      if re.search('\s'+re.escape(ellie_cats[e][i])+'\s',comp):
        new_schools[e][j] = 1
print("Creating '../bok_courses_cat_ellie.csv'...")
new_schools.to_csv('../bok_courses_cat_ellie.csv',index=False)
